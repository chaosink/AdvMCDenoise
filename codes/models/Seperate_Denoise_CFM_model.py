...
class Seperate_Denoise_CFM_Model(BaseModel):
    def __init__(self, opt):
        super(Seperate_Denoise_CFM_Model, self).__init__(opt)
        train_opt = opt['train']
    ...
    def feed_data_diffuse(self, data, need_GT=True):
        ...
        self.var_L = data["diffuse_in"].to(self.device)
        #self.var_L = torch.randn(1, 3, 100, 100, requires_grad=True).to(self.device)
        self.var_H = data["diffuse_ref"].to(self.device)
    ...
    def optimize_parameters(self, step):
        ...
        if self.opt['train']['gan_type'] == 'wgan-gp':
            ...
            interp_crit = self.netD(interp) #, _
